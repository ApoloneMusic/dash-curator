...
export const spotifyService = {
  ...
  getClientCredentialsToken: async (): Promise<string> => {
    ...
    if (clientCredentialsToken && tokenExpiryTime && Date.now() < tokenExpiryTime) {
    ...
    try {
      ...
      throw new Error(error instanceof Error ? error.message : "Failed to authenticate with Spotify")
    }
  },
  ...
  getUserPlaylists: async (userId: string): Promise<SpotifyPlaylist[]> => {
    try {
      ...
      const response = await fetch(`${SPOTIFY_API_BASE_URL}/users/${userId}/playlists?limit=50`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  getPlaylistArtworkByUrl: async (playlistUrl: string): Promise<string | null> => {
    try {
  ...
  searchPlaylists: async (query: string): Promise<SpotifyPlaylist[]> => {
    ...
    try {
      ...
      const response = await fetch(
        `${SPOTIFY_API_BASE_URL}/search?q=${encodeURIComponent(query)}&type=playlist&limit=50`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
