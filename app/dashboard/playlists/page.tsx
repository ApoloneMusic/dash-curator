...
export default function PlaylistsPage() {
  ...
  const fallbackArtworkImages = ["/electronic-playlist.png", "/indie-playlist-cover.png", "/hip-hop-playlist.png"]
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [playlistsResponse, genresResponse, subgenresResponse] = await Promise.all([
        ...
        const curatorPlaylists = playlistsData.filter((playlist: Playlist) => playlist.curators_id === Number(user.id))
        ...
        fetchPlaylistArtwork(curatorPlaylists)
      } catch (error) {
  ...
  const fetchPlaylistArtwork = async (playlistsToUpdate: Playlist[]) => {
    setIsLoadingArtwork(true)

    try {
      // Create an array of promises for fetching artwork
      const artworkPromises = playlistsToUpdate.map(async (playlist) => {
        try {
          // Only fetch if the playlist has a URL and doesn't already have artwork
          if (playlist.playlistUrl && !playlist.artwork) {
            const artworkUrl = await spotifyService.getPlaylistArtworkByUrl(playlist.playlistUrl)
            return {
              id: playlist.id,
              artwork: artworkUrl,
            }
          ...
          console.error(`Error fetching artwork for playlist ${playlist.id}:`, error)
          return null
        }
      })
      ...
      setPlaylists((prevPlaylists) => {
        return prevPlaylists.map((playlist) => {
          const artworkResult = artworkResults.find((result) => result && result.id === playlist.id)
          if (artworkResult && artworkResult.artwork) {
            return { ...playlist, artwork: artworkResult.artwork }
          }
          return playlist
        })
      })
  ...
  const getSubgenreNames = (subgenreIds: number[]) => {
    return subgenreIds.map((id) => subgenres.find((subgenre) => subgenre.id === id)?.name).filter(Boolean)
  }
  ...
  const handleUpdatePlaylist = async (updatedPlaylist: PlaylistData) => {
    ...
    try {
      ...
      const originalPlaylist = playlists.find((p) => p.id === updatedPlaylist.id)
      if (!originalPlaylist) {
      ...
      const updateData = {
        ...
        subgenres: subgenreIds.length > 0 ? subgenreIds : originalPlaylist.subgenres,
        description: updatedPlaylist.description || originalPlaylist.description,
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/playlists/${updatedPlaylist.id}`, {
        method: "PATCH",
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || `Failed to update playlist: ${response.status}`)
      }
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to update playlist. Please try again.",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
  ...
  const handleDeletePlaylist = async (playlistId: number) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to delete playlist. Please try again.",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
  ...
  const openEditModal = (playlist: Playlist) => {
    ...
    const playlistData: PlaylistData = {
      ...
      artwork: playlist.artwork || fallbackArtworkImages[playlist.id % fallbackArtworkImages.length],
  ...
  const refreshPlaylists = async () => {
    ...
    try {
      ...
      // Filter playlists by curator ID
      const curatorPlaylists = data.filter((playlist: Playlist) => playlist.curators_id === Number(user.id))
      ...
      const playlistsNeedingArtwork = updatedPlaylists.filter((p) => !p.artwork)
      if (playlistsNeedingArtwork.length > 0) {
        fetchPlaylistArtwork(playlistsNeedingArtwork)
      }
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
        <div className="flex flex-col md:flex-row md:items-center gap-4">
          <Button variant="outline" size="sm" asChild className="flex items-center gap-1 w-fit">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1">Manage and track your Spotify playlists</p>
          </div>
        </div>
        <div className="flex gap-2 mt-4 md:mt-0">
          ...
          >
            <RefreshCw className={`h-4 w-4 ${isLoading || isLoadingArtwork ? "animate-spin" : ""}`} />
            Refresh
          </Button>
          <Button size="sm" className="flex items-center gap-1 glow-button" onClick={() => setIsAddModalOpen(true)}>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground mt-1">You don't have any playlists yet. Add a playlist to get started.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-4">
          {playlists.map((playlist) => (
            <PlaylistCard
              ...
              followers={playlist.saves}
              tracks={Math.floor(Math.random() * 50) + 10} // Random number of tracks for demo
              status={playlist.status as "active" | "inactive" | "pending" | "archived" | "unverified" | "verified"}
